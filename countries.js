...
let country; // Declare the country variable in a broader scope

// Fetch the JSON file
fetch('countries.json')
  .then(response => response.json()) // Parse the JSON response
  .then(data => {
    // Assign the fetched data to the country variable
    country = data;
  ...
  data.forEach(country => {
    if (country.independent === true) {
      ...
      row.innerHTML = `
          ...
          <td class='capital-name'>${country.capitalName}</td>
          <td>${country.flag1x1}</td>
          <td>${country.isoCode}</td>
          <td class='continent-name'>${country.continentName}</td>
        `;
      tableBody.appendChild(row);
      i++;
    }
...
const toggleBtn = document.getElementById("toggleBtn");
    ...
    toggleBtn.addEventListener("click", () => {
      ...
      const capitalNameElements = document.querySelectorAll(".capital-name");
      const continentNameElements =
        document.querySelectorAll(".continent-name");

      countryNameElements.forEach((element) => {
        const originalText = element.textContent;
        const translatedText = country.countryNamePt;
        element.textContent = translatedText;
        element.dataset.translatedText = originalText; // Store original text in dataset
      });

      capitalNameElements.forEach((element) => {
        const originalText = element.textContent;
        const translatedText = country.capitalNamePt;
        element.textContent = translatedText;
        element.dataset.translatedText = originalText; // Store original text in dataset
      });

      continentNameElements.forEach((element) => {
        const originalText = element.textContent;
        const translatedText = country.continentNamePt;
        element.textContent = translatedText;
        element.dataset.translatedText = originalText; // Store original text in dataset
      });
});
